...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams LEFT JOIN games ON teams.team_id=games.winner_id LEFT JOIN games AS o ON teams.team_id=o.opponent_id WHERE games.year=2014 OR o.year=2014 ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT(w.name) FROM games LEFT JOIN teams AS w ON games.winner_id=w.team_id ORDER BY w.name")"
